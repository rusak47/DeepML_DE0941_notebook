...
# TODO - identify correct sizes
W_3 = np.random.randn(4,1)
b_3 = np.zeros(1,) # 1d

# Linear(x,W,b) = W . x + b <- reusing
...
# y' = Model (x, W_1, b_1, W_2, b_2, W_3, b_3) = Linear(
#                                                       tanh(
#                                                               Linear(
#                                                                       tanh(Linear(x, W_1, b_1))
#                                                                      , W_2, b_2)
#                                                           )
#                                                       , W_3, b_3)
def model(x, W_1, b_1, W_2, b_2, W_3, b_3):
    ...
    out = linear(W_2, b_2, out)
    out = tanh(out)
    out = linear(W_3, b_3, out)
    return out

# todo why yprim - y? (formula have it reversed, i.e. y - y_prim). for abs there is no diff, but prim_loss sign will change
def loss_mae(y_prim, y):
    out = np.mean(np.abs(y_prim -y))
    return out

# todo is the order of y_prim and y correct here?
def dy_prim_loss_mae(y_prim, y):
    out = (y_prim - y)/ (np.abs(y_prim-y) + 1e-8)
    return out #TODO

def loss_mse(y_prim, y):
    out = np.mean((y_prim-y)**2)
    return out

# 1/n sum (y-y')**
def dy_prim_loss_mse(y_prim, y):
    out = np.mean(-2 * (y - y_prim) )
    return out
...
def dx_tanh(x):
    exp_neg2x= np.exp(-2*x)
    out = ( 2 * exp_neg2x / (1 + exp_neg2x) ) * ( 1 + ( 1 - exp_neg2x) / ( 1+ exp_neg2x) )
    return out
