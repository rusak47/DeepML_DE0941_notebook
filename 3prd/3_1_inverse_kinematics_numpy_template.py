...
"""
def angle_between_projection(p0, p1, p_proj_0 = None, p_proj_1 = None):
    if p_proj_0 is None:
...
"""
    exp_slope_coeff - control how fast exponential value reaches near to zero value (1 for close to straight line)
    speedup_coeff - smaller value results in smaller changes
"""
def fading_coeff_exponent(normalized_angle, y_offset = 0, speedup_coeff=50, exp_slope_coeff=1):
    return (((np.exp(exp_slope_coeff*normalized_angle) -1)/(np.exp(exp_slope_coeff) - 1)) + y_offset ) * speedup_coeff
...
while is_running:
    ...
    print(f"angle t1: {np.rad2deg(t1_angle)}")  # 90* perpendicular, 0* parallel, so we want angle to be above 0
    ...
    if abs(loss) > eps:#todo dont allow to increase loss value -> predict next?
        ...
        """
            ...
            in std gradient descent the gradient is computed for all the data;
        ...
        fading_coeff_fn = fading_coeff_exponent(t1_angle_norm, speedup_coeff=beta)
        ...
        if v_length3 <= 0:
            # reaches 1st segment at angle 180 from both sides
            t2_angle = np.deg2rad(180) - angle_between_projection(p1, p2, x, p1)
            theta2_norm = t2_angle / 180  # [0,1]
            fading_coeff2 = fading_coeff_exponent(theta2_norm, speedup_coeff=50)  # *np.sign(theta2_norm)
            theta_2 -= d_theta_2 * alpha * fading_coeff2 # TODO doesnt save from crossing x-axis, maybe fade by y coordinate...
                                                            # anyway it's kind of workaround, not the real solution, so i'd stop here...
