...
while is_running:
    ...
    if abs(loss) > eps:  # todo dont allow to increase loss value -> predict next?
        ...
        if False:
            print(f"-----------------")
            print(
                f"origin: {(d_theta_1 * alpha):.4f}; new: {(d_theta_1 * alpha * fading_coeff_fn):.4f}; e:{fading_coeff_fn:.4f}")
        ...
        if v_length3 <= 0:
            ...
            theta_2 -= d_theta_2 * alpha * fading_coeff2
            print(f"-----------------")
            print(f"angle t2: {np.rad2deg(t2_angle)}")
            print(f"origin: {(d_theta_2 * alpha):.4f}; new: {(d_theta_2 * alpha * fading_coeff2):.4f}; e:{fading_coeff2:.4f}")
            print(f"-----------------")
