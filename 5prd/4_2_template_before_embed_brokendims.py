...
class Dataset:
    def __init__(self):
        ...
        print(f"{os.path.exists(path_dataset)}")
        if not os.path.exists(path_dataset):
            os.makedirs('./data', exist_ok=True)
            ##features: name yearselling_pricekm_drivenfuelseller_typetransmissionOwner
        ...
        self.X[:, -2], _, _= self.normalize(self.X[:, -2])
    ...
    def normalize(self, x):
        ...
        x_min = np.min(x, axis=0) # across the values
        x_max = np.max(x, axis=0)

        x_norm = (x-x_min)/ (x_max -x_min) # range [0,1]
        x_norm = (x_norm - 0.5 )*2 # scale to [-1,1]
...
class DataLoader:
    ...
    def __len__(self): # how many batches in dataset
    ...
    def __next__(self):
        ...
        x,y = self.dataset[idx_start:idx_end]#passing a range
        self.idx_batch +=1
...
class LayerLinear:
    def __init__(self, in_features: int, out_features: int):
        ...
        self.b: Variable = Variable(
            value=np.random.randn(out_features),  #todo check is that correct
            grad= np.zeros(shape=(BATCH_SIZE, in_features,out_features))
        )
        self.x: Variable = None
        self.output: Variable = None

    def forward(self, x: Variable): # calculating actual values
        self.x = x
        W_dot_x = (self.W.value.T @ x.value[:,:,None])[:,:,0] # .squeeze()
    ...
    def backward(self): # have output, so here we calculate the amount of change -> gradient
        ...
        self.x.grad += (self.W.value @ self.output.grad[:, :, None])[:,:,0] # squeeze removes last dimension (alternative)
...
class LossMAE():
    ...
    def backward(self): # derivative impl
...
for epoch in range(1, 1000):
    ...
    for dataloader in [dataloader_train, dataloader_test]:
        ...
        for x, y in dataloader:
            ...
            #todo find where from this bug comes?
